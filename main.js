...
// planet object
class planet {
    constructor(speed, radius, orbitRadius, orbitAngle, rotation, mesh) {
        this.speed = speed;
        this.radius = radius;
...
// sun
const sun = new planet(0, 80, 0, 0, .01 * (1 / 27),
    new THREE.Mesh(
...
// earth
const earth = new planet(.0001, 2, sun.radius + 92.96, 0, .01,
    new THREE.Mesh(
...
// mercury
const mercury = new planet(earth.speed * (1 / .241), earth.radius * (2 / 5), sun.radius + 35.98, 0, earth.rotation * (1 / 59),
    new THREE.Mesh(
...
// venus
const venus = new planet(earth.speed * (1 / .616), earth.radius * .949, sun.radius + 67.24, 0, earth.rotation * (1 / 117),
    new THREE.Mesh(
...
// mars
const mars = new planet(earth.speed * (1 / 1.88), earth.radius * .532, sun.radius + 141.6 * .75, 0, earth.rotation * (1 / 1.026),
    new THREE.Mesh(
...
// jupiter
const jupiter = new planet(earth.speed * (1 / 12), earth.radius * 11, sun.radius + 483.8 * .3, 0, earth.rotation * 2.4,
    new THREE.Mesh(
...
// saturn
const saturn = new planet(earth.speed * (1 / 29), earth.radius * 9.45, sun.radius + 890.8 * .3, 0, earth.rotation * 2.24,
    new THREE.Mesh(
...
// uranus
const uranus = new planet(earth.speed * (1 / 84), earth.radius * 4.007, sun.radius + 1784 * .3, 0, earth.rotation * 1.39,
    new THREE.Mesh(
...
// neptune
const neptune = new planet(earth.speed * (1 / 65), earth.radius * 3.883, sun.radius + 2793 * .3, 0, earth.rotation * 1.49,
    new THREE.Mesh(
...
// pluto
const pluto = new planet(earth.speed * (1 / 248), earth.radius * .186 * 10, sun.radius + 3700 * .3, 0, earth.rotation * (1 / 6.4),
    new THREE.Mesh(
...
function buttonPress(button) {
    switch (button.id) {
        case '1':
            changePivot(mercury.mesh);
            break;
        case '2':
            changePivot(venus.mesh);
            break;
        case '3':
            changePivot(earth.mesh);
            break;
        case '4':
            changePivot(mars.mesh);
            break;
        case '5':
            changePivot(jupiter.mesh);
            break;
        case '6':
            changePivot(saturn.mesh);
            break;
        case '7':
            changePivot(uranus.mesh);
            break;
        case '8':
            changePivot(neptune.mesh);
            break;
        case '9':
            changePivot(pluto.mesh);
            break;
        case '0':
...
function drawOrbit(planet) {
    ...
    l.rotateX((Math.PI / 2) + planet.orbitAngle);
    scene.add(l);
}
...
function orbit(planet) {
    ...
    planet.mesh.position.set(
        ...
        0, -Math.sin(date) * planet.orbitRadius
    );
}
